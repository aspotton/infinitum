# Infinitum v0.2.11 Architecture
...
The compiled block is designed to be cache-stable. Within one session the block is pinned byte-for-byte and only invalidated when memory or topic state actually changes, tracked by a watermark over the memories and topics tables. The pin applies only when the client supplies a session id (session header or request metadata); a request without one is compiled fresh under a generated, provenance-only id and never enters the block cache. The memory message is injected immediately before the last user message by default (`context.inject_position: suffix`), leaving the system prompt and the conversation history ahead of it byte-identical for upstream prompt caches; `inject_position: prefix` restores injection after the leading system messages for strict chat templates; because the injected memory message uses the default `memory_message_role: system`, strict chat templates that raise on any system message not at index 0, or providers that reject mid-list system messages, must set `inject_position: prefix`. When `memory.tools_enabled` is on, the two drill-down tool definitions are exposed statically on memory-enabled requests unless the client defines tools with the same names, so the tools region never flickers between turns.
A topic-summary refresh that produces an identical summary does not bump the invalidation watermark; memory-write updates still do, by design.

Echo protection is deliberately asymmetric: raw `request.received` events keep exactly what the client sent, while derived interaction text used for later recording and learning has echoed `<infinitum_memory>` regions stripped so quoted context never re-enters memory. That split is intentional and should not be harmonized away.
...
The loop runs on the foreground path while background learning stays on its usual path afterwards: learning is enqueued exactly once per HTTP request, at final completion. Intermediate rounds never reach the client or the assistant event stream. Each suppressed call is persisted as a `memory.tool_call` event whose metadata carries the reconstructed assistant message (including its `tool_calls`) as the audit trail. The loop caps at 4 rounds; any tool call naming a function other than the two Infinitum tools is terminal and forwarded verbatim. With the memory-namespace guard active, a round classifies on its tool-call structure rather than on content, so plain text sitting beside an Infinitum tool call suppresses the round and the loop continues instead of the text ending it. The same guard wires mixed-round stripping into every forwarded byte-yield site (accum replay, live tail, mid-round consume output, held-burst flush, and buffered/forced replays), so a forwarded stream carries no `infinitum_` bytes while `completed()` and the recorded events stay on raw bytes. The client-visible rule is visible text, invisible machinery: the client sees only its own tool calls, and client-defined names are always forwarded even when they begin with `infinitum_`.
